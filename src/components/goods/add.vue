      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px" label-position='top'>
        <el-tabs v-model="activeIndex" :tab-position="tabPosition" :before-leave="beforeTabLeave" @tab-click="tabClicked">
          ...
          <el-tab-pane label="商品图片" name="3">
            <!-- action指定图片上传的api地址-->
            <el-upload :action="upLoadURL" :on-preview="handlePreview" :on-remove="handleRemove" list-type="picture" :headers="headersObj" :on-success="handleSucce">
              <el-button size="small" type="primary">点击上传</el-button>
            </el-upload>
          </el-tab-pane>
          <el-tab-pane label="商品内容" name="4">
            <quill-editor v-model="addForm.GoodsContent" />
            <!-- 添加按钮 -->
  ...
  methods: {
    ...
    handleChange() {
      if (this.addForm.goods_cat.length !== 3) {
        this.addForm.goods_cat = []; //如果没有选择三级分类，重置为空
      }
    },
    ...
    addGoods(){
      ...
      this.$refs.addFormRef.validate(async (valid) => {
          ...
          // 准备请求前的数据
          //处理商品所属的数据
          //this.addForm.goods_cat = this.addForm.goods_cat.join(',') 级联选择器要求必须是数组，变成字符串以后级联选择器会失效
          //定义一个新的对象，将addForm拷贝过来
          //使用lodash工具实现深拷贝
